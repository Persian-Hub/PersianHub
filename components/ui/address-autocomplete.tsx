...
export function AddressAutocomplete({
  ...
  useEffect(() => {
    if (isLoaded && inputRef.current && !autocompleteRef.current && !disabled) {
      // <CHANGE> Enhanced autocomplete configuration with better place selection handling
      ...
      autocompleteRef.current.addListener("place_changed", () => {
        ...
        // <CHANGE> Enhanced place validation and feedback
        if (place.formatted_address && place.geometry) {
          setIsPlaceSelection(true)
          setShowFeedback(true)
          
          // <CHANGE> Preserve existing form state by only updating address-related fields
          console.log("[v0] Address selected from autocomplete:", place.formatted_address)
          onChange(place.formatted_address, place)
          
          // <CHANGE> Show visual feedback for 2 seconds
          setTimeout(() => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // <CHANGE> Only call onChange for manual typing, not when Google Places is setting the value
    if (!isPlaceSelection) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // <CHANGE> Prevent form submission when selecting from autocomplete dropdown
  ...
  return (
    <div className="relative">
      <div className="relative">
        ...
        {/* <CHANGE> Map pin icon when loaded */}
        {isLoaded && !isLoading && (
          <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
      ...
      {/* <CHANGE> Visual feedback when address is selected */}
      {showFeedback && (
